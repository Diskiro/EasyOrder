...
export default function StaffManagement() {
    ...
    const handleCreateUser = async (e: React.FormEvent) => {
        ...
        try {
            // 1. Create a temporary client to avoid switching the current admin session
            const supabaseUrl = import.meta.env.VITE_SUPABASE_URL
            const supabaseAnonKey = import.meta.env.VITE_SUPABASE_ANON_KEY

            // Create a client that does NOT persist session to storage
            ...
            const { data: { user }, error: signUpError } = await tempClient.auth.signUp({
                email: newUserEmail,
            ...
            if (!user) throw new Error('Falló la creación del usuario (no se devolvió usuario)')

            // 3. If Role is NOT 'waiter' (default), update it immediately using Admin privileges
            if (newUserRole !== 'waiter') {
                ...
                if (roleError) {
                    console.warn('Could not update role immediately, check manual update.', roleError)
                    showAlert('Usuario creado, pero falló la actualización del rol. Por favor actualízalo manualmente.', 'warning')
                }
            }
